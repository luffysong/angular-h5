       ...
       /* if($scope.ids && $scope.type == "balance" && $scope.calAmount > 0){
        ...
        $scope.skipPay = function(){
            ...
            if($scope.payType == 'alipay'){
                location.href = '//'+location.host+'/p/payment/4/send-payment-request?'+(['pay_type=D','trade_id='+$scope.tid,'url_order=https:'+encodeURIComponent($scope.rongHost+'/m/#/zhongchouAllOrder'),'back_url=https:'+encodeURIComponent($scope.rongHost+'/m/#/zhongchouAllOrder'),'coupon_ids='+$scope.ids]).join('&');
            }else {
                if(!$scope.hasRecord){
