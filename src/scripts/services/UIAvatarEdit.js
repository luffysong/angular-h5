...
angular.module('defaultApp.service').directive('imgCropped', function ($timeout) {
    ...
    return {
        ...
        link: function (scope, element) {
            var myImg,_myImg, clear = function () {
                if (myImg) {
                    myImg.next().remove();
                    myImg.remove();
                    myImg = undefined;
            ...
            return $modal.open({
                ...
                controller: function ($scope, $timeout, $upload, $modalInstance, fileReader, DefaultService) {
                    ...
                    $scope.ok = function () {
                        ...
                        switch(rotate){
                            case 90:
                                var x = $scope._picHeight -crop.y-crop.height;
                                var y = crop.x;
                                crop.x = x;
                                crop.y = y;
                                break;
                            case 180:
                                var y = $scope._picHeight - crop.y - crop.height;
                                var x = $scope._picWidth - crop.x -crop.width;
                                crop.x = x;
                                crop.y = y;
                                break;
                            case 270:
                                var x = crop.y;
                                var y = $scope._picWidth - crop.x -crop.width;
                                crop.x = x;
                                crop.y = y;
                                break;
                            default :
